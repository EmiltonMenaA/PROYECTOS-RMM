...
async function checkSchema() {
  try {
    ...
    if (projects.rows.length > 0) console.log(JSON.stringify(projects.rows, null, 2));
    
    console.log('\n---\n');
    
    // Check supervisors
    const supervisors = await db.query('SELECT id, email, role FROM users WHERE role = $1 LIMIT 3', ['supervisor']);
    console.log(`Supervisors in DB: ${supervisors.rows.length}`);
    if (supervisors.rows.length > 0) console.log(JSON.stringify(supervisors.rows, null, 2));
    
    console.log('\n---\n');
    
    // Check reports
    const reports = await db.query('SELECT id, project_id, author_id, summary, created_at, status FROM reports LIMIT 3');
    console.log(`Reports in DB: ${reports.rows.length}`);
    if (reports.rows.length > 0) console.log(JSON.stringify(reports.rows, null, 2));
